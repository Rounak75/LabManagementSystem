...
export const Select = forwardRef<HTMLSelectElement, Props>(({ label, error, className = "", children, ...rest }, ref) => (
  <label className="block">
    ...
    <select ref={ref} className={`w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm focus:border-brand focus:outline-none ${error ? "border-danger" : ""} ${className}`} {...rest}>
      {children}
    </select>
